...
| `byproduct/numeric_audit.py` | Not a unit suite: 179 numerical checks against the **live** market. CBOE and Yahoo priced against each other, our implied vols against the ones CBOE publishes, our SPY variance-swap rate against the published VIX, greeks against finite differences, BKM against the density, the density's mean against the forward, touch probabilities against terminal ones, realised-vol estimators against the measured overnight share of variance, and facts with outside answers — CUSIPs, the next NVDA announcement, Berkshire's largest 13F position |
